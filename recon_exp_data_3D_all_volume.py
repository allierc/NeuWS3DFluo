...
if __name__ == "__main__":
    ...
    target = imread(f'./DATA/target.tif')          # ground truth fluorescence volume 256 256 256 ZXY Z=0 top Z=255 bottom obective below plane z=255
    dn = imread(f'./DATA/dn_reversed.tif')         # ground truth refractive index (real part only) volume 256 256 256 ZXY Z=0 top Z=255 bottom obective below plane z=255
    loss = 0

    for gridsearch in range(10):                    # grid search running to find hyperparameters
        ...
        net = net.to(DEVICE)
        state_dict = torch.load('./DATA/model_epoch_3584.pt')              # First model trained with fluo only
        ...
        for epoch in range(512):
            ...
            for it in it_list:          # for every plane loss calculated for 20 introduce aberrations
