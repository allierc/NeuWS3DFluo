...
class bpm3Dfluo_all_volume(torch.nn.Module):

    def __init__(self, bpm_config):

        super(bpm3Dfluo_all_volume, self).__init__()
        self.mc = bpm_config
    ...
    def forward(self, plane=None, phi=None, gamma=None, fluo_unknown=None, dn_unknown=None):
        ...
        self.dn0_layers = dn_unknown.unbind(dim=0)

        for i in range(plane,self.Nz):
            if (i==plane):
                S = torch.mul(fluo_unknown,torch.exp(phi*1.j))
                self.field = torch.fft.ifftn(torch.mul(torch.fft.fftn(S),self.C))
    ...
    def FresnelPropag(self, dz=0, fdir=[0, 0]):
        ...
        return hz
class bpm3Dfluo_defocus(torch.nn.Module):

    def __init__(self, bpm_config):

        super(bpm3Dfluo_defocus, self).__init__()
        self.mc = bpm_config
    ...
    def FresnelPropag(self, dz=0, fdir=[0, 0]):
        ...
        return hz
class bpm3Dfluo_PSF(torch.nn.Module):

    def __init__(self, bpm_config):

        super(bpm3Dfluo_PSF, self).__init__()
        self.mc = bpm_config
    ...
    def forward(self, plane=None, phi=None, gamma=None, fluo_unknown=None, dn_unknown=None):
        ...
        self.field = torch.mul(S,torch.exp(torch.mul(fftshift(dn_unknown), 1.j)))
        self.field = torch.mul(self.field,gamma)

        I = torch.abs(torch.fft.ifftn(self.field)) ** 2  # aberration is defined per volume (x256)
