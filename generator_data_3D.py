...
if __name__ == '__main__':
    ...
    if bSingle == True:
        ...
        x_batches = torch.zeros((1, 256, 256, 256), dtype=torch.cfloat, device=device)
        y_batches = torch.zeros((1, 256, 256, 256), device=device)

        for plane in range(0, bpm.Nz):

            print(f'plane: {plane}')

            out_cpx = torch.zeros(1, 256, 256, dtype=torch.cfloat, device=DEVICE)
            ...
            Istack[:, :,0:1] = I[:,:,None].detach().cpu().numpy() / Niter
            ...
            imwrite(f'./Pics_input/stack_single/input_aberration_plane{plane}.tif', tmp)
